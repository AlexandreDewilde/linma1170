...
static void p1_stifness_matrix (double const dphi[6], double const h[3][3] ,double const det, double S[6][6]){
	...
	double const d1dy = dphi[1];
	double const d2dx = dphi[2];
	double const d2dy = dphi[3];
	double const d3dx = dphi[4];
	double const d3dy = dphi[5];

	double const BT[3][6] = {{d1dx, d2dx, d3dx, 0, 0, 0},
...
static void hooke_plain_stress(double const E, double const nu, double h[3][3]) {
	double const C = E / (1 - nu * nu);
	h[0][0] = h[1][1] = C;
	h[2][2] = C * (1 - nu);
	h[0][1] = h[1][0] = C * nu;
...
void p1_geometry(double const* x, double *detptr, double dxidx[2][2], double* dphi) {
	double const dxdxi[2][2] = {
...
void p1_laplace_matrix(double const alpha, double const dphi[6], double const det, double S[3][3]) {
	double const d1dx = dphi[0];
	double const d1dy = dphi[1];
	double const d2dx = dphi[2];
	double const d2dy = dphi[3];
...
int get_triangles_p1(size_t** elementTags, size_t* elementTags_n, size_t** nodeTags, size_t* nodeTags_n) {
  	int ierr;
  	gmshModelMeshGetElementsByType(2, elementTags, elementTags_n, nodeTags, nodeTags_n, -1, 0, 1, &ierr);
  	return ierr;
}

int get_nodes (size_t ** nodeTags, size_t * nodeTags_n, double** coord, size_t * coord_n) {
  	int ierr;
...
Matrix* compute_mass(size_t const nNodes, double const* coord, size_t const ntriangles, size_t* const triangleNodes, double const rho) {
	...
	for (size_t i = 0 ; i < ntriangles ; i++){
		...
		double det, dxidx[2][2], dphi[6];
		p1_geometry(x,&det, dxidx, dphi);
		double const A = rho * det / 12;
		double const B = A / 2.0;
...
void get_boundary_nodes(size_t** bnd_nTags, size_t* bnd_nTags_n){
	...
	for (size_t i=0;i<dimTags_n;i+=2){
		char *name = malloc(525*sizeof(char)); // Should be deleted because gmsh allocate name
		char *name_sec = name;
	...
	for (size_t i=0; i<*bnd_nTags_n; i++)
		(*bnd_nTags)[i]--;
	if(dimTags)
		free(dimTags);
}
